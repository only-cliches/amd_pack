...
if (process.argv[2] == "build") {
    ...
    (async () => {
        ...
        for (let k in styles) {
            ...
            style_files.push({size: file_contents.length / 1000, file: file_name.split(path.sep).pop(), sri: get_file_hash(path.join(__cwd, "libs", module_name || package, file_name.split(path.sep).pop()))});
        }
