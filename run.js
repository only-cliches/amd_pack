...
(async () => {
    ...
    await new Promise((res, rej) => {
        const prod = child.spawn(path.join(__dirname, `node_modules/.bin/webpack-cli`), [`--progress`, `--config`, `webpack.config.prod.5.js`, `--env`, `type=prod`, `mod=${package}`], {
            cwd: process.cwd(),
            detached: true,
            stdio: "inherit"
          });
    ...
    // non minified build
    await new Promise((res, rej) => {
        const prod = child.spawn(path.join(__dirname, `node_modules/.bin/webpack-cli`), [`--progress`, `--config`, `webpack.config.prod.5.js`, `--env`, `type=dev`, `mod=${package}`], {
            cwd: process.cwd(),
            detached: true,
            stdio: "inherit"
          });
